...
in {
  ...
  config = mkMerge [
    (mkIf
      (cfg.enable && lib.hasAttrByPath [ "i3status-rust" ] config.programs) {
            ...
            blocks = [
              {
                ...
                block = "custom";
                command =
                  "${pkgs.myPkgs.khal-events-json}/bin/khal-events-json";
                json = true;
                interval = 30;
              ...
              }
              {
                ...
                interval = 60;
                inboxes = [
                  "${config.home.homeDirectory}/Mail/protonmail/Inbox"
                  "${config.home.homeDirectory}/Mail/work/Inbox"
                ];
                threshold_warning = 1;
