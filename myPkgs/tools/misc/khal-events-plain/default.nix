...
pkgs.writeShellScriptBin "khal-events-plain" ''
  ...
  if [[ $UPNEXT == "No events" ]]; then
    ...
    MINTUESUNTIL="$(( ($(${pkgs.coreutils}/bin/date +%s -d "today $(${pkgs.khal}/bin/khal list "now" "eod" -f "{start-time}" | ${pkgs.gnugrep}/bin/grep "^[^ ]" | ${pkgs.coreutils}/bin/head -2 | ${pkgs.coreutils}/bin/tail -1)") - $(${pkgs.coreutils}/bin/date +%s)) / 60 )) $UPNEXT";
  fi
